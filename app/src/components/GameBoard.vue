...
export default { 
  ...
  setup(){
    ...
    house3rent, house4rent, hotelrent, eachhousecost) => {
     ...
     const communitychest = SpaceObjects(null,"Community Chest", )
   ...
   let spacesArray = [go, mediterennan, communitychest, balticavenue, incometax, readingRailRoad, 
     ...
     return {spacesArray, username, freeParking, bottomRow, getItemClass }
...
<template>
  ...
  <div  id ="bottom-row"  class ="grid grid-cols-11  col-start-1  row-start-[-1] col-end-12 relative h-[9vh]   ">
    <div v-for="(space, index) in bottomRow" :key="space.id" >
      
   
  <div :class="getItemClass(index)" class ="pt-2 pb-2" >
  </div>
    <div class ="space-name text-xs mt-4">{{  space.name }}</div>
  
  <h1 class ='mt-9'>{{ space.price }}</h1>
  </div>
